import rtde_control
from libs.robotiq_preamble import ROBOTIQ_PREAMBLE
import time
...
class RobotiqGripper(object):
    ...
    def wait_obj_detection(self):
        """ 
        Wait till a object is detected.
           
        Returns: 
            True if the command succeeded, otherwise it returns False
        """
        return self.call("OBJ_DET", "rq_wait_for_object_detected()")

    def is_obj_detected(self):
        """ 
        Returns the state of object detection.
           
        Returns: 
            True if the command succeeded, otherwise it returns False
        """
        return self.call("IS_OBJ_DET", "rq_is_object_detected()")

    def get_obj_var(self):
        """
        Get register value for OBJ

        returns:
            set of byte values
        """
        return self.call("GET_OBJ", "rq_get_var(OBJ, 1, gripper_socket)")
